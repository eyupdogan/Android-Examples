...
class LoginInformationActivity : AppCompatActivity()
{
    ...
    private fun successLoginsButtonClickedCallback()
    {
        ...
        } catch (ignore: Throwable)
        {
            runOnUiThread {
                Toast.makeText(
                    this,
                    "Problem occurred. Try again later",
                    Toast.LENGTH_SHORT
                ).show()
            }
        }
    ...
    private fun failLoginsButtonClickedCallback()
    {
        try
        {
            ...
            val logins =
                dataService.findFailLoginInfoByUsername(mLoginInfo.username)
            if (logins.isNotEmpty())
                runOnUiThread {
                    logins.forEach {
                        mBinding.adapter!!.add(it)
                    }
                }
            else
                runOnUiThread { Toast.makeText(this, "No fail login", Toast.LENGTH_SHORT).show()}
    ...
    private fun initBinding()
    {
        ...
        mBinding.adapter =
            ArrayAdapter(this, android.R.layout.simple_list_item_1, ArrayList<LoginInfoDTO>())
    }
    ...
    fun loginInformationItemClicked(pos: Int)
    {
        ...
        AlertDialog.Builder(this).setTitle(R.string.alertdialog_login_info_title_text)
