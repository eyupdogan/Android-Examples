...
class RegisterActivity : AppCompatActivity()
{
    ...
    private class RegisterHandler(activity: RegisterActivity):Handler(Looper.myLooper()!!)
    {
        ...
        private fun handleUserNotRegistered()
        {
            ...
            Toast.makeText(activity, "${activity.mBinding.user!!.username} cannot be registered", Toast.LENGTH_SHORT).show()
        }
        private fun handleRegisterSuccess()
        {
            ...
            Toast.makeText(
                mWeakReference.get(),
                "${activity.mBinding.user!!.username} successfully registered",
        ...
        override fun handleMessage(msg: Message)
        {
            val activity = mWeakReference.get()!!
            when(msg.what){
                WHAT_REGISTER_BIRTHDATE_INVALID ->
                    Toast.makeText(activity, "Invalid date format", Toast.LENGTH_SHORT).show()
                WHAT_REGISTER_USER_NOT_REGISTERED ->
                    handleUserNotRegistered()
                WHAT_REGISTER_DATA_SERVICE_EXCEPTION ->
                    Toast.makeText(activity, "Data problem: ${msg.obj}", Toast.LENGTH_SHORT).show()
                WHAT_REGISTER_EXCEPTION ->
                    Toast.makeText(activity, "Problem occurred. Try again later", Toast.LENGTH_SHORT).show()
    ...
    {
        AlertDialog.Builder(this).setTitle(R.string.alertdialog_register_close_title_text)
            ...
            .setNegativeButton(R.string.alertdialog_register_close_no_button_text) {_,_->}.create().show()
    }
}