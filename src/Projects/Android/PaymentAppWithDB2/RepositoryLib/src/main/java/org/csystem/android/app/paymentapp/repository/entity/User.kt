...
data class User(
    ...
    @ColumnInfo(name = "register_date") var registerDate: LocalDate = LocalDate.now()
) : Serializable
{
    constructor(username: String, password: String,firstname: String,
        lastName: String,  birthDate: LocalDate = LocalDate.now(),
        registerDate: LocalDate = LocalDate.now()) : this(username,password, firstname,
        null, lastName, birthDate, registerDate)
}
