...
class MainActivity : AppCompatActivity() {
    ...
    private fun initBinding() {
        ...
        mBinding.apply {
            viewModel = MainActivityListenerViewModel(this@MainActivity)
    ...
    fun calculateButtonClicked() {
        if (intBinaryOperator.isValid((mBinding.operationInfo!!.op))) {
            mBinding.resultText = intBinaryOperator.applyAsInt(
                mBinding.operationInfo!!.first,
                mBinding.operationInfo!!.second
            ).toString()
        }else
